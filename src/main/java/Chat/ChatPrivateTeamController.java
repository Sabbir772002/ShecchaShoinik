...
//        }
    ...
    String role2="";
    int f=0;
     public void set(String username, String role, String name2, String user2, BorderPane pane,String role2) {
        this.pane = pane;
        //user.setText(username);
       // rolee.setText("@" + role);
        this.role = role;
 ...
 /*   class chatthread extends Thread {
    ...
    void refresh(ActionEvent e){
        ...
        try {
            ...
            String sql;
            if(f==0) {
                sql ="SELECT * FROM privatechat Where Sender = ? OR Sender = ? OR Reciver = ? OR Reciver = ?";
            }else {
                sql = "SELECT * FROM privateteam Where Sender = ? OR Sender = ? OR Reciver = ? OR Reciver = ?";
    ...
    public void send(ActionEvent e) {
       ...
       // refresh();
        try {
            ...
            String st="";
            if(f==0){
      st = "INSERT INTO privatechat (Sender, Reciver, Msg) VALUES (?,?,?)";}
            else{
                st = "INSERT INTO privateteam (Sender, Reciver, Msg) VALUES (?,?,?)";
            }
